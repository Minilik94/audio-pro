...
const ProductListings = (product: { product: ProductType | null }) => {
  ...
  console.log(product.product, "from listing");
  ...
  return (
    <div className="max-w-[69.375rem] mx-auto h-full shadow-none relative overflow-hidden font-manrope px-2 w-full">
      {product?.product && product.product.length > 0 ? (
        product?.product.map((product: ProductType, index: number) => (
          <div className="lg:my-20 mb-8" key={product.slug}>
            <div className="grid grid-cols-1 lg:grid-cols-2 h-full w-full px-8">
              <div
              ...
              ></div>
              <div className="flex flex-col justify-center gap-9 px-24 py-2">
                <p
                  className={`${
                    product.new
